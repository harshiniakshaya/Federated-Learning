...
import { hospitalA_local, hospitalA_comm } from "../api"
...
export default function HospitalA() {
  ...
  const addPatient  = () => run("Adding Patient",    async () => {
    ...
    return hospitalA_local.post("/add_patient", payload)
  })
  const trainLocal  = () => run("Training Model",    () => hospitalA_local.post("/train_local"))
  const pushModel   = () => run("Pushing to Central",() => hospitalA_comm.post("/push_to_central"))
  const pullModel   = () => run("Pulling Global",    () => hospitalA_comm.get("/pull_from_central"))
    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-100 tracking-tight">Hospital A</h2>
        ...
        <div className="text-right">
          ...
          <div className="text-sm font-mono text-cyan-400 mt-0.5">HOSP-A-001</div>
        </div>
      </div>
        ...
        <div className="p-6">
          {mode === "form" ? (
              ...
              {FIELDS.map((f) => (
                <div key={f.name} className="flex flex-col gap-1">
                  ...
                  <input
                    ...
                    className="bg-slate-950 border border-slate-700 rounded-md px-3 py-2 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 transition-all"
            ...
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-700 rounded-md px-4 py-3 text-sm text-cyan-300 font-mono placeholder-slate-600 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 transition-all resize-none"
              placeholder={'{\n  "age": 45,\n  "bp": 80,\n  ...\n}'}
            />
          )}
          ...
          <button
            onClick={addPatient}
            disabled={loading}
            className="mt-4 px-6 py-2.5 bg-cyan-500 hover:bg-cyan-400 disabled:opacity-40 disabled:cursor-not-allowed text-slate-950 text-xs font-black tracking-widest uppercase rounded-lg transition-all shadow-lg shadow-cyan-500/20"
          >
            + Add Patient
          </button>
        </div>
        ...
        <div className="p-6 flex flex-wrap gap-3">
          {[
            ...
            <button
              ...
              `}
            >
              {loading && activeAction === label.split(" ")[0] + " " + label.split(" ")[1]
                ? "Processing..."
                : label}
            </button>
          ))}
      ...
      {(loading || output) && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
          ...
          <pre className="p-6 text-xs text-cyan-300 font-mono overflow-auto max-h-80 leading-relaxed">
